...
sledgelib_func unsigned int ContainerCreate(unsigned int iWidth, unsigned int iHeight, int iX, int iY) {
	unsigned int iHandle = static_cast<unsigned int>(WebContainers::RegisteredContainers.size());
...
sledgelib_func void ContainerDestroy(unsigned int iHandle) {
	CWebContainer* Container = WebContainers::RegisteredContainers.at(iHandle);
	if (!Container) return;
