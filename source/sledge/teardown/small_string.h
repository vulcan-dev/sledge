...
public:
	...
	small_string(const char* cStr)
	{
		...
		if (lLenght > 15)
		{
			cDest = static_cast<char*>(Teardown::alloc(lLenght + 1));
			m_HeapBuffer = cDest;
			m_StackBuffer[15] = 1;
		}
	...
	void operator=(small_string ssSource){
		...
		{
			this->m_HeapBuffer = static_cast<char*>(Teardown::alloc(ssSource.len() + 1));
	...
	inline void resize(size_t lNewSize)
	{
		...
		if (!this->m_StackBuffer[15])
		{
			char* pNewHeapBuff = static_cast<char*>(Teardown::alloc(lNewSize));
